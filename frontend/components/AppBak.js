        ...
        return (
            <div className={classes.root}>
                        ...
                        <form className={classes.addForm}
                            ...
                            <div>
                                <Button className={classes.modalBtn}
                                    onClick={this.handleAddClose}>取消</Button>
                                <Button className={classes.modalBtn}
                                    type="submit"
                ...
                <main className={classes.content}>
                    <IconButton onTouchMove={this.handleDrag}
                        onTouchStart={this.handleDragStart}
                        onTouchEnd={this.handleDragEnd}
                        className={classNames(classes.dragBtn, {[classes.dragBtnHover]: longTouch})}
                        style={{left: `${positionX}%`, top: `${positionY}%`}}>
                        <DragIcon className={classes.dragBtnIcon}/>
                    </IconButton>
                    <div className={classes.add}
                        style={{left: `${positionX}%`, top: `${positionY}%`}}>
                        <Button onClick={this.handleAddFormOpen(1)}
                            className={classes.addBtn} />
                        <Button onClick={this.handleAddFormOpen(2)}
                            className={classes.addBtn} />
                        <Button onClick={this.handleAddFormOpen(3)}
                            className={classes.addBtn} />
                        <Button onClick={this.handleAddFormOpen(4)}
                            className={classes.addBtn} />
                    </div>
                    <GridList className={classes.gridList}>
                        <GridListTile className={classNames(classes.gridListTitle, classes.gridListTitle1)}
                            style={{width: `${positionX}%`, height: `${positionY}%`}}
                            key="1">
                            <List>
                                <ListSubheader>重要紧急</ListSubheader>
                                {do1.map(item => (<ListItem className={classNames({[classes.thingComplete]: item.complete})}
                                    onClick={this.handleComplete(item)}
                                    key={item.uuid}>{item.name}</ListItem>))}
                            </List>
                        </GridListTile>
                        <GridListTile className={classNames(classes.gridListTitle, classes.gridListTitle2)}
                            style={{width: `${100 - positionX}%`, height: `${positionY}%`}}
                            key="2">
                            <List>
                                <ListSubheader>重要不紧急</ListSubheader>
                                {do2.map(item => (<ListItem className={classNames({[classes.thingComplete]: item.complete})}
                                    onClick={this.handleComplete(item)}
                                    key={item.uuid}>{item.name}</ListItem>))}
                            </List>
                        </GridListTile>
                        <GridListTile className={classNames(classes.gridListTitle, classes.gridListTitle3)}
                            style={{width: `${positionX}%`, height: `${100 - positionY}%`}}
                            key="3">
                            <List>
                                <ListSubheader>不重要紧急</ListSubheader>
                                {do3.map(item => (<ListItem className={classNames({[classes.thingComplete]: item.complete})}
                                    onClick={this.handleComplete(item)}
                                    key={item.uuid}>{item.name}</ListItem>))}
                            </List>
                        </GridListTile>
                        <GridListTile className={classNames(classes.gridListTitle, classes.gridListTitle4)}
                            style={{width: `${100 - positionX}%`, height: `${100 - positionY}%`}}
                            key="4">
                            <List>
                                <ListSubheader>不重要不紧急</ListSubheader>
                                {do4.map(item => (<ListItem className={classNames({[classes.thingComplete]: item.complete})}
                                    onClick={this.handleComplete(item)}
                                    key={item.uuid}>{item.name}</ListItem>))}
                            </List>
                        </GridListTile>
