  ...
  _onSwipe(locationX) {
    ...
    // if (Math.abs(swipeWitdh) > SCREEN_WIDTH / 4) {
      if (swipeWitdh > 0) {
        ...
        nextTabWith = oldActiveTab + 1 > 3 ? this.state[WIDTH_NAME[oldActiveTab + 1]] + swipeWitdh * 2 : this.state[WIDTH_NAME[oldActiveTab + 1]];
      }else {
        if (oldActiveTab - 1 > -1) {
          nextTab = oldActiveTab - 1;
        }
        activeTab = oldActiveTab - 1 > -1 ? oldActiveTab - 1 : 4;
        nextTabWith = oldActiveTab - 1 > -1 ? this.state[WIDTH_NAME[oldActiveTab - 1]] + swipeWitdh * 2 : this.state[WIDTH_NAME[oldActiveTab - 1]];
      }
    // }
    this.setState({
    activeTab,
  ...
  render() {
    return (
      <Container style={styles.container}>
        ...
        <View
        style={{flexDirection: "row", flex: 1, width: "100%", height: "100%"}}
